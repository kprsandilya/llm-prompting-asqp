...
from modified_dataloader import ModifiedDataLoader
from llm import LLM
import itertools
import json
import random
...
def zero_shot(TASK, DATASET_NAME, DATASET_TYPE, LLM_BASE_MODEL, SEED, MODE, N_FEW_SHOT, SORT_EXAMPLES):
    ...
    fs_examples_txt = ""
    with open(f"./datasets/{TASK}/{DATASET_NAME}/train.txt", "r", encoding="utf-8") as f:
        lines = f.readlines()
...
for combination in combinations:
    ...
    file_path = f"generations/nurse/{task}_{dataset_name}_{dataset_type}_{model.split(':')[0]}_{seed}_{mode}_{fs}.json"
